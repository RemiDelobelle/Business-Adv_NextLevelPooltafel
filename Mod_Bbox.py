...
from Mod_Constants import PRINTS, THRESHOLD_STRIPED_BALLS, MARGIN_STRIPED_BALLS

def process_bbox(bbox, clean_img, mask_stripedBalls, projection_MASK):
    print(f"Processing bbox: {bbox}") if PRINTS else None
    print(f"Clean image: {type(clean_img)}, {clean_img}") if PRINTS else None
    ...
    print(f"Xbox1: {xbox1}, Ybox1: {ybox1}, Xbox2: {xbox2}, Ybox2: {ybox2}") if PRINTS else None
    roi = clean_img[ybox1:ybox2, xbox1:xbox2]           #TODO: Check if the coordinates are correct/ are striped balls being detected???
    print(f"ROI: {roi.shape}, {roi}") if PRINTS else None
    if roi is not None:
    ...
    return mask_stripedBalls, projection_MASK

def process_bboxes(bbox_coor, clean_img, mask_stripedBalls, projection_MASK):
    with ThreadPoolExecutor() as executor:
        for bbox_args in bbox_coor:
            future = executor.submit(process_bbox, bbox_args, clean_img, mask_stripedBalls, projection_MASK)
            mask_stripedBalls, projection_MASK = future.result()
